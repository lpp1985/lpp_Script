...
def BWA_MAPPING( file_list  ):
	...
	def get_outputname( each_f ):
		output_preifx = path+'/'+name +'.' + re.search( '(read\d+)',each_f ).group(1)
		return output_preifx
	def BWA_sai_Run( each_f ,output_preifx ):
		
		
		os.system(  ' bwa aln -o 1 -t %s   -q 60 %s %s -f %s.sai 1>%s.log 2>&1 ' %( 
	...
	if not os.path.exists(  path  ):
		os.makedirs( path )
	sorted_file = sorted(  file_list,key = lambda x:  int( re.search( 'read(\d+)'   ,x ) .group(1)    )  )
	...
	os.system(  'bwa sampe  -P -N 5 %s %s  %s  %s  %s -f %s.sam 1>%s_PE.log 2>&1'%(  ref , out_hash[ read1_file   ]+'.sai'  , out_hash[ read2_file ]+'.sai' ,read1_file ,read2_file ,output_preifx ,output_preifx )    )
...
for a,b,c in os.walk(  inputpath ):
	for each_f in c:
		if each_f.endswith(  '.fastq.pair_end' ):
			name = each_f.split('.')[0]

...
name_trans= {}
l=0
TRANS = open("Name_Trans.tsv",'w')
def get_primer( (t,s) ):
	global l,TRANS
	
	title =re.search( '>([^\n]+)',t ).group(1)
	if title not in name_trans:
		TRANS.write("%s\t%s\n"%(title,l))
		name_trans[title ] = str(l)
		l+=1 
	s = re.sub('\s+','',s)
	all_gap_loc =  [[200,len(s)-200]]

	path = root_path+'/primer_input1/%s/'%( name_trans[title ] )
	check_path(path)
	primer_end = path+'%s.primer'%(name_trans[title ])
	primer_raw = path+'%s.primer3'%(name_trans[title ])
	primer_log = path+'%s.log'%(name_trans[title ])
	...
	i=0
	for gap_start,gap_stop in all_gap_loc:
...
=
'''%( name_trans[title],s,'%s,%s'%( gap_start,length  ),product_min,product_max,Tm_upper,Tm_dwon,Tm_optimum, Tm_diff, gc_upper,gc_down  ) )
		LOG.write( '''%s\t%s\t%s\t%s\n'''%( name_trans[title],gap_start,gap_stop,length ) )
	os.chdir( path )
	subprocess.call(  ['primer3_core',primer_raw,'-output', name_trans[title]+'.primer'],stdout= subprocess.PIPE)
	...
	for e_b in RAW:
		i+=1
		gap_id = re.search( 'SEQUENCE_ID=(\S+)',e_b ).group(1)

...
def BWA_MAPPING( file_list  ):
	...
	def BWA_sai_Run( each_f ,output_preifx ):
		...
		os.system(  ' bwa aln -o 1 -t %s  -I -q 60 %s %s -f %s.sai 1>%s.log 2>&1 ' %( 
	...
	os.system(  'bwa sampe -s  -P -N 5 %s %s  %s  %s  %s -f %s.sam 1>%s_PE.log 2>&1'%(  ref , out_hash[ read1_file   ]+'.sai'  , out_hash[ read2_file ]+'.sai' ,read1_file ,read2_file ,output_preifx ,output_preifx )    )

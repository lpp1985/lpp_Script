...
class fastq_quality_class(  fastq_check    ):
	...
	for x in xrange(-10,65):
		phred_64_quality_hash[ chr( x+64  ) ] = x
	@staticmethod
	...
	def __init__( self,**kword  ):
		...
		self.N = 0
		self.Q20 = 0
		self.low_complex_data = 0.0
		self.stats_path = root_path+'/stats/'
		self.check_path(  self.stats_path  )	
		self.kmer = kword['kmer']
		self.filtered = kword[ 'filtered'  ]
		
		self.tolerate = kword[  'tolerate'   ]
	...
	def KmerQC( self,each_block   ):
		kmer_all,kmer_hash = K_Mer( each_block[1].strip(),self.kmer   )
		if len( kmer_hash )/ float( len( kmer_all )  ) >=self.tolerate:
			out_hash['KmerData']  = self.static( each_block    )[-1]
	...
	def trimQC(  self,each_block ):
		...
		m=0
		for each_q in quality[:-1]:
			
			if self.phred_64_quality_hash[ each_q  ]>=self.quality:
				break
			else:
				m+=1
		trim_start = m
		n= len( quality[:-1] )
		for each_q in quality[:-1][::-1]:
			if self.phred_64_quality_hash[ each_q  ]>=self.quality:
				break
			else:
				n-=1
		...
		if self.length<1:
			depend = len( quality[:-1][ trim_start: trim_stop   ] ) / float( len( quality[:-1]  ) )
		else:
			depend = len( quality[:-1][ trim_start: trim_stop   ] )
		if  depend >=self.length:
			#[ trim_start,trim_stop ]     = seq_re.span(1)

			seq_trim     = seq[:-1][ trim_start:trim_stop  ]
			quality_trim = quality[:-1][ trim_start : trim_stop  ]
	...
	def each_qc(  self, each_iter  ):
		...
		def check( each_hash ):
			global GC,N,Q20,raw_data_count,raw_reads_count_iner,filter_read_count,filter_data_count,trim_read_count,trim_data_count,filter_detail_cache,low_complex_data
			GC +=each_hash[ 'RAW_GC' ] 
			...
			if 'failKmer' in each_hash:
				

				low_complex_data +=len( each_hash['failKmer'][1].strip() )
				#return ''
	...
	def outputStatic(self):
		...
		if self.filtered:
			self.low_complex_data_perc = 100*self.low_complex_data/float(self.raw_data_count )
		else:
			self.low_complex_data_perc='-'
		...
		if self.trim:
			...
			self.stats_title = '#Sample\tCategory\tRawReads\tRawData(MB)\tLow_complexity\tTrimReads\tTrimData(MB)\tTrimReadPerc(%)\tTrimDataPerc(%)\tFilterReads\tFilterData(MB)\tFilterReadsPerc(%)\tFilterDataPerc(%)\tN(%)\tGC(%)\tQ20(%)'
			output_rank  = [
			...
			self.stats_title = '#Sample\tCategory\tRawReads\tRawData(MB)\tLow_complexity\tFilterReads\tFilterData(MB)\tFilterReadsPerc(%)\tFilterDataPerc(%)\tN(%)\tGC(%)\tQ20(%)'
			output_rank  = [
...
def get_paramater(  ):
	try:
		...
		parser.add_option(
		        ...
		        default= False,
		        dest="filter",
		        help="Do you want to filter the fastq file( filter low complexity and tandem repeat reads  )"
		                  )
		...
		parser.add_option(
		        ...
		        default= 0.75,
		        dest="tolerate",
		        help="The k-mer percentage to be recognized as low complexity"
				                  )		
		...
		parser.add_option("-k", "--Kmer", action="store",
		                  ...
		                  type='int',
		                  default=4,
		                  help="Kmer size to consider low complexity"
		                  )			
		...
		parser.add_option("-l", "--Trim_length", action="store",
		                  ...
		                  type= "float",
		                  default=60,
		                  help='''The length of threshold  to  filter after trim, 
						  ...
						  THIS PARAMATER OUGHT TO BE INGORANCE IF [-t] IS MISSING!!!''')
		...
		kmer = options.kmer
		return path , all_raw_file , quality , threshold , trim , trim_length,block_size,cache_size, filtered, tolerate,kmer
	except:

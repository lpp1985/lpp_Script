...
def bowtie_check(  each_group  ):
	...
	sample_name = os.path.split(  each_group[0]  )[-1].split('.')[0]
...
def getpara(  ):
	...
	try:
		...
		kmer = options.kmer		
		return path , all_raw_file , quality , threshold , trim , trim_length,process, thread,catlength,block_size,cache_size,filtered,tolerate,kmer
	except:
...
if __name__=='__main__':
	# get the paramater
	path , all_raw_file , quality , threshold , trim , trim_length,process, thread,catlength,block_size,cache_size,filtered,tolerate,kmer= getpara()

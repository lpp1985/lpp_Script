...
def run(x):
	x=str(x)
	command = 'soapdenovo63mer all -s %sK'%( outputpath )+x+'/config'+x+'.ini -K'+x+' -R -o %sK'%( outputpath )+x+'/soap_k_'+x+' -p '+ str( cpu )+ ' >%srun'%( outputpath )+x+'.log '
	os.system( command)
def assembly(   ):
	...
	if new:
		for i in Kmer:
			check_path( '%sK%s'%(outputpath,i) )
	need_r = Kmer
	for i in need_r:
		CONFIG = open('%sK%s/config%s.ini'%( outputpath,i,i ),'w')

...
for line in MATRIX:
	...
	for i in xrange(0,len(express_l) ):	
		rpk = 10**3*(float(express_l[i]))/unigene_length[gene_name]
		all_RPK[ title_l[i] ].append(rpk)
		
for key,data in all_RPK.items():
	sum_RPK[key] = sum(data)/10**6
MATRIX.seek(0,0)
MATRIX.next()
for line in MATRIX:
	line_l = line.strip().split("\t")
...
# plt.savefig("%s_TPM.png"%(options.graph))
# plt.show()

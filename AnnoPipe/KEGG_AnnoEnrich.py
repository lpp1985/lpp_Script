...
if __name__=="__main__":
	...
	parser.add_option("-a", "--AllAlign", action="store", 
	                  dest="AllAlign", 
	                  help="All Alignment Data")
	
	parser.add_option("-d", "--Enrich", action="store", 
	                  dest="Diff", 
	                  help="Diff List")	
...
"""
	...
	ALL_Align = open(options.AllAlign, 'rU')
	alldiff_data = File_dict( open( options.Diff, 'rU')).read(1, 1)
	DIFF = open( "%s.align" % (os.getpid() ), 'w')
	DIFF.write( ALL_Align.next())
	for line in ALL_Align:
		line_l = line.split("\t")
		if line_l[0] in alldiff_data:
			DIFF.write(line)
		
	
	blast_mapping_command = config_hash["Utils"]["gapmap"]+'/blast_sql.py -f %(all)s   -r  %(diff)s   -1 forward -2 forward  -n Forward -N Reverse -p %(pep)s -d %(dna)s -x %(tag)s -q'%(
	    {
	        "all":options.AllAlign,
	        "diff": DIFF.name,
	        "pep":proteinseq,
	        "dna":options.NUL,
	        "tag":tag
	...
	source_command = config_hash["Utils"]["gapmap"]+"/Mapping_sql.py -t -o %s -d %s"%(
	    source_location,
	    tag,
	...
	pathway_stats_command = config_hash["Utils"]["gapmap"]+"/Pathway_statsDiff.py %(name)s %(out)s_PathwayCategoery.tsv %(out)s_PathwayCategory_Stats.stat"%(
	    {
	                "name":tag,
	                "out":output_prefix
	            }
	...
	database_generate_commandline = " KEGG_Database.py  -a %s  -p %s  -d %s_KEGG.xls  "%(  DIFF.name,"%s_pathway.tsv"%(output_prefix) ,output_prefix   )

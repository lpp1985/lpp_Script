	...
	for each_f in f:
		if '.png' in each_f:
			...
			if re.search('(pair\d+)',each_f):
				id = re.search('pair(\d+)',each_f).group(1)
				output_hash[ name ][ 'corrected' ][ 'read%s'%( id )  ][ tag  ]= path +each_f
			elif 'filter' in each_f:
				output_hash[ name ][ 'filter' ][ read  ][ tag  ]= path +each_f
			else:
				output_hash[ name ][ 'raw' ][ read  ][ tag  ]= path +each_f
		if '.filter.stats' in each_f:
			
			RAW = open( path+each_f,'rU' )
			
			other_hash[ name ]['filterreads_content'] = RAW.next()[:-1]
			
			other_hash[ name ]['filterreads_number'] = RAW.next()[:-1]
			
		elif '.pair1.fastq.stats' in each_f:
			
			RAW = open( path+each_f,'rU' )
			
			other_hash[ name ]['Kmer_content'] = RAW.next()[:-1]
			
			other_hash[ name ]['Kmer_number'] = RAW.next()[:-1]
		elif 'read1.fastq.stats' in each_f:
			RAW = open( path+each_f,'rU' )
			other_hash[ name ]['raw_content'] = RAW.next()[:-1]
			other_hash[ name ]['raw_number' ] = RAW.next()[:-1]
...
for title in output_hash:
	...
	word.Selection.Find.Execute('%raw_content%', False, False, False, False, False, True, 1, True, other_hash[ title ]['raw_content'], 2)
	word.Selection.Find.Execute('%raw_number%', False, False, False, False, False, True, 1, True, other_hash[ title ]['raw_number'], 2)
	word.Selection.Find.Execute('%Kmer_content%', False, False, False, False, False, True, 1, True, other_hash[ title ]['Kmer_content'], 2)
	#word.Selection.Find.Execute('%Kmer_content%', False, False, False, False, False, True, 1, True, '1000', 2)
	#print(  other_hash[ title ]['Kmer_content'] )
	word.Selection.Find.Execute('%Kmer_number%', False, False, False, False, False, True, 1, True, other_hash[ title ]['Kmer_number'], 2)
	
	word.Selection.Find.Execute('%filterreads_content%', False, False, False, False, False, True, 1, True, other_hash[ title ]['filterreads_content'], 2)
	word.Selection.Find.Execute('%filterreads_number%', False, False, False, False, False, True, 1, True, other_hash[ title ]['filterreads_number'], 2)
		...
		if each_staus == 'raw':
			...
			for each_read in output_hash[ title  ][ each_staus  ]:
				if each_read=='read1':
					for each_tag in output_hash[ title  ][ each_staus  ][ each_read ]:
						if each_tag =='qualstats':
							MS_Word_Find_Past_Image(word, '#Read1_KQ#',  output_hash[ title  ][ each_staus  ][ each_read ] [ each_tag ])
						else:
							MS_Word_Find_Past_Image(word, '#Read1_KD#',  output_hash[ title  ][ each_staus  ][ each_read ] [ each_tag ])
				else:
					for each_tag in output_hash[ title  ][ each_staus  ][ each_read ]:
						if each_tag =='qualstats':
							MS_Word_Find_Past_Image(word, '#Read2_KQ#',  output_hash[ title  ][ each_staus  ][ each_read ] [ each_tag ])
						else:
							MS_Word_Find_Past_Image(word, '#Read2_KD#',  output_hash[ title  ][ each_staus  ][ each_read ] [ each_tag ])

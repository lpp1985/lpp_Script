...
def generate_unitig(sg, seqs, out_fn, connected_nodes = None):
    ...
    while len(sg_edges) > 0:
        ...
        if whole_path:
            ...
            first_name = whole_path[0]
            seq_name = first_name.split(":")[0]
            if first_name.endswith("E"):
                seq = seqs[seq_name]
            else:
                seq = complement(seqs[seq_name])
            subseqs=[seq]
...
def is_branch_node(G, n):
    ...
    for i in range(len(n2)):
        for j in range(i+1, len(n2)):
            v = n2[i]
            w = n2[j]
            if neighbor_bound(G, v, w, 2) == False:
                is_branch = True
                break
        if is_branch == True:
            break
...
if __name__ == "__main__":
    ...
    with open(overlap_file) as f:
        for l in f:
            ...
            if identity < 95: # only take record with >96% identity as overlapped reads
                continue
            if score > -1000:
                continue

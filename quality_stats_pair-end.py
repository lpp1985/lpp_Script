...
class fastq_quality_pair(  object    ):
	...
	def __init__(  self,**kword  ):
		...
		if not len(   kword[  'files' ] ) ==2:
			print(   'Data %s is error!!!'%(  '\t'.join(  kword[  'files' ]  )   )    )
			raise ValueError
	...
	def next( self  ):
		...
		try:
			...
			try:
				...
				self.FilterReads =  self.read1.filter_read_count +self.read2.filter_read_count

				self.FilterData =   self.read1.filter_data_count +self.read2.filter_data_count
...
def PairDbRun(  **paramater ):
	...
	for x in QCPAIR_class:
		pass
	return QCPAIR_class.RawReads , QCPAIR_class.RawData ,  QCPAIR_class.FilterReads  , QCPAIR_class.FilterData, QCPAIR_class.FilterPerc, QCPAIR_class.N_perc , QCPAIR_class.GC_perc, QCPAIR_class.Q20_perc

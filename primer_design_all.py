...
for t,s in RAW:
	...
	for each_Gap in re.finditer('([n|N]+)',s1,re.I):
		...
		ALL_CAND.write(  '; '.join( tag_contain )+'\t'+'; '.join(  [x+'F' for x in tag_contain ]  )  +'\t'+primer_f+'\t'+str(product_start)+'\t'+str( primer_Len )+'\t'+ tm_f+'\t'+ G_C_P_f+'\t'+product_size+'\t'+'; '.join( gap_region )+'\tprimer\n\t')
		ALL_PRIMER.write(  '; '.join(  [x+'F' for x in tag_contain ]  )  +'\t'+primer_f+'\n'  )
		ALL_CAND.write('; '.join(  [x+'R' for x in tag_contain ]  )  +'\t'+primer_r+'\t'+str(gap_stop)+'\t'+str( primer_Len )+'\t'+tm_r+'\t'+G_C_P_r+'\t'+product_size+'\t'+'; '.join( gap_region )+'\tprimer\n')

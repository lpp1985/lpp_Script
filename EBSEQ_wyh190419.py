...
pdf(file="%(out_prefix)s_Volcano.pdf")
ggplot(Res,aes(Log2FC,-log10(FDR),col=Condition))+geom_point()+ylab("-log10(FDR)")+theme_few()+xlab("log2FC")+scale_colour_manual(values=c("green", "blue", "red"))+ guides(colour = guide_legend(title = "FDR<0.05 and |log2Foldchange|>=1"))+theme(legend.position=c(.2, .9))

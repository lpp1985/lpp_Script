...
func main() {
	...
	db_kmer.EnsureIndex(index)
	var db_seq = session.DB(*name).C("Sequence")
	db_seq.EnsureIndex(index)
	...
	for {
		...
		all_seq = append(all_seq, &Sequence{Name: name + "+", Seq: string(seq)})
		all_seq = append(all_seq, &Sequence{Name: name + "-", Seq: string(lpp.RevComplement(seq))})
		x += 2
	...
	kmer_bulk := db_kmer.Bulk()
	cache := make([]interface{}, len(kmer_graph))
	i := 0
	for query, hash := range kmer_graph {
		cache[i] = &Kmer{Name: query, Son: hash}
		i += 1

	}

	kmer_bulk.Insert(cache...)
	kmer_bulk.Unordered()
	kmer_bulk.Run()

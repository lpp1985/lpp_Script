...
func main() {
	...
	for _, e_s := range all_sample {
		fmt.Println(e_s)
		RAW, _ := os.Open(e_s + ".fa")
		data, _ := ioutil.ReadAll(RAW)
		OUTPUTBUF.Write(data)
	}
	//	for k, sample := range all_sample {
	//		OUTPUTBUF.WriteString(">" + sample + "\n")
	//		OUTPUTBUF.WriteString(all_fasta[k].String() + "\n")
	//	}
}

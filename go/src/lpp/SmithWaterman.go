...
func COORD_MERGE(array [][2]int) int {
	...
	for _, data := range array {
		if data[0] > max {
			length += int(math.Abs(float64(data[1]-data[0])) + 1)
			max = data[1]
		} else {
			if data[1] > max {
				length += int(math.Abs(float64(data[1]-max)) + 1)
				max = data[1]
			}
...
func COORD_CHAIN(array [][2]int, raw_array [][2]int, gap int, status int) ([][2]int, [][2]int, int) {
	...
	for i = 1; i < length; i++ {
		...
		for j = 0; j < i; j++ {
			if raw_array[i][0]-raw_array[j][1] > gap {
				continue
			}
			if status == 0 {

				if array[j][1] <= array[i][0] {
					cache := liss[j] + raw_array[i][1] - raw_array[i][0]

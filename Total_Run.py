...
import subprocess
import os
from quality_stats_Pairend_Distri_33 import get_paramater,PairEndRun,get_sample,fastq_quality_pair
...
def bowtie_check(  each_group  ):
	...
	if len( all_file_group[ each_group ]  )>1:
		...
		read1file = all_file_group[ each_group ][0]
		bowtie_subprocess = subprocess.Popen(
				        [
				                'bowtie',
...
def getpara(  ):
	...
	try:
		...
		if version ==1:
			...
			entroy = 33		
		return path , all_raw_file , quality , threshold , trim , trim_length,process, thread,catlength,block_size,cache_size,filtered,tolerate,kmer,entroy
	except:
...
def Total_Run(  each_group ):
	...
	if  QUEUE.qsize():
		...
		raise ValueError, each_group+' BWA error!  '
	if 'IOError' in data:
		raise IOError , data[-1]
	else:
		insertsize = data
...
if __name__=='__main__':
	# get the paramater
	path , all_raw_file , quality , threshold , trim , trim_length,process, thread,catlength,block_size,cache_size,filtered,tolerate,kmer,entroy= getpara()

...
sub training{
  ...
  if(!uptodate([$genome,"$rootDir/good.gtf"],[$genbank])  || $overwrite){
    print STDOUT "NEXT STEP: create genbank file\n";
    $errorfile = "$errorfilesDir/gff2gbSmallDNA.stderr";

    $perlCmdString = "perl $string $rootDir/good.gtf $genome $flanking_DNA $genbank 2>$errorfile";

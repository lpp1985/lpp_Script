...
def GBLASTA( protein,assemblyresult,output ):
    ...
    os.system( """Genblasta.nf --query=%s --db="./Assem" --out="%s" """ % (protein,output ))
def ParseGblasta(gbaresult,genewiseruncommand):
    ...
    for e_b in GBA:
        ...
        for align in gb_block:
            ...
            if perc >=80:
                ...
                commandline = """genewise %s  %s   -gff -silent -quiet  -sum    """%(os.path.abspath(PRO.name),os.path.abspath(NUC.name))
                if direct =="-":
                    commandline += "-trev"
    ...
    os.system("Genewise.nf --command %s --out %s"%(COMMAND.name , outputpath))

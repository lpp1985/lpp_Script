...
if __name__ == '__main__':
	...
	for line in INPUT:
		...
		if q_name not in all_has or s_name not in all_has[q_name]:
			
			all_has[q_name][s_name] = [[q_start, q_end]]
		else:
			all_has[q_name][s_name].append( [q_start, q_end])
	all_length = {}
	for t, s in fasta_check( open(options.fasta)) :
		name = t.split()[0][1:]
		all_length[name] = len(s.replace("\n", ""))
	for q , q_hash in all_has.items():
		for s in q_hash:
			all_data = sorted(q_hash[s], key = lambda x: x[0] )
			result_data = []

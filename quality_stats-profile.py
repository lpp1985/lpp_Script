...
def fastx_filter_check(  reads,end_path ):
	...
	STATS.write(  os.popen(  '''aa=`wc -l %s| grep -Po '^\d+'` &&echo "scale=3;$aa/4*%s/1024/1024"|bc  && echo "$aa/4"|bc  '''%( end_title , length_reads  )  ).read()  )
...
def fastx_check(  reads,end_path ):
	...
	length_reads = len( RAW.next()[1][:-1]  )
	STATS = open( '%s.stats'%( end_title ),'w' )
	STATS.write(  os.popen(  '''aa=`wc -l %s| grep -Po '^\d+'` &&echo "scale=3;$aa/4*%s/1024/1024"|bc  && echo "$aa/4"|bc  '''%( reads , length_reads  )  ).read()  )
...
for prefix in  name_hash:
	...
	for key1 in glob.glob(  end_path+'*.fastq.other'    ):

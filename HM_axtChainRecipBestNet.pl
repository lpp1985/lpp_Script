...
sub rbestNet {
	...
	$cmd .= "| chainSort /dev/stdin $path/$query.$target.$net_name.tbest.chain 2>>$path/_ChainSort.log";
	print $cmd,"\n";
	system($cmd);
	...
	$cmd .= "| chainSort /dev/stdin /dev/stdout > $path/$target.$query.$net_name.rbest.chain 2>>$path/_ChainSort.log";
	print $cmd,"\n";
	system($cmd);
	...
	$cmd .= "$target.seq $query.seq $path/$target.$query.$net_name.rbest.axt";
	print $cmd,"\n";
	system($cmd);
	# axt to chain
	my $options;
	...
	if ($Arg_list =~ m/--qrbest/){
		...
		$cmd .= "| chainSort /dev/stdin /dev/stdout > $path/$query.$target.$net_name.qrbest.chain 2>>$path/_ChainSort.log ";
		print $cmd,"\n";
		system($cmd);
